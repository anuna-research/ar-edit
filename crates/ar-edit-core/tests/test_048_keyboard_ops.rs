...
use ar_edit_core::models::{
    Defaults, EditDocument, EditOpKind, Manifest, ShotRange, Source, Transcript, TranscriptSegment,
    Word,
};
...
fn trim_shot_updates_range() {
    ...
    doc.trim_shot("shot-001", ShotRange::Words { from: 2, to: 7 })
        .unwrap();
    assert_eq!(
        doc.snapshot.shots[0].range,
        ShotRange::Words { from: 2, to: 7 }
    );
}
...
fn trim_nonexistent_shot_errors() {
    let mut doc = EditDocument::create("test");
    let err = doc
        .trim_shot("shot-999", ShotRange::Words { from: 0, to: 5 })
        .unwrap_err();
    assert!(format!("{err}").contains("not found"));
}
...
fn ops_survive_save_load() {
    ...
    .unwrap();
    doc.move_shot("shot-001", 1).unwrap();
    doc.trim_shot("shot-001", ShotRange::Words { from: 1, to: 4 })
        .unwrap();
