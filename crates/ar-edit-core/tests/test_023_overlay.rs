...
fn overlay_integration_timecode_offset_accumulates() {
    ...
    let f0 = filters[0].as_ref().unwrap();
    assert!(f0.contains("pts:hms:0.000"), "first shot offset should be 0");
...
fn overlay_integration_minimal_mode_timecode_only() {
    ...
    assert_eq!(f.matches("drawtext=").count(), 1, "minimal should have single drawtext");
}
...
fn overlay_integration_snippet_from_transcript() {
    ...
    assert!(f.contains("Welcome to the interview"), "should include transcript snippet");
    assert_eq!(f.matches("drawtext=").count(), 2, "should have 2 drawtext filters (info + snippet)");
}
...
fn overlay_integration_no_snippet_for_time_range() {
    ...
    assert_eq!(f.matches("drawtext=").count(), 1, "time range should have 1 drawtext (no snippet)");
...
fn overlay_integration_all_filters_have_valid_drawtext_syntax() {
    ...
    for (i, filter) in filters.iter().enumerate() {
        ...
        assert!(f.starts_with("drawtext="), "filter {i} should start with drawtext=");
        assert!(f.contains("text='"), "filter {i} should have single-quoted text");
        assert!(f.contains("fontsize="), "filter {i} should specify font size");
        assert!(f.contains("fontcolor=white"), "filter {i} should use white text");
        assert!(f.contains("boxcolor=black@0.5"), "filter {i} should have semi-transparent bg");
        assert!(f.contains("x=10"), "filter {i} should be positioned at x=10");
        assert!(f.contains("y=10"), "filter {i} should be positioned at y=10");
    }
}
