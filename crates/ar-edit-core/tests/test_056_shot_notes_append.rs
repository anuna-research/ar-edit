...
fn notes_on_different_shots() {
    ...
    doc.add_note("shot-001", "Actually keep the opening")
        .unwrap();
    ...
    assert_eq!(
        doc.snapshot.shots[0].notes[1].text,
        "Actually keep the opening"
    );
}
...
fn notes_persist_through_save_load() {
    ...
    doc.add_shot("src-001", ShotRange::Words { from: 0, to: 50 })
        .unwrap();
    doc.add_note("shot-001", "Director's note: perfect delivery")
        .unwrap();
